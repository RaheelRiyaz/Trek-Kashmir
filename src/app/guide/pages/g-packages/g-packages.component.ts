...
@Component({
  selector: 'app-g-packages',
  templateUrl: './g-packages.component.html',
  styleUrls: ['./g-packages.component.css']
})
export class GPackagesComponent {
  ...
  getPackages(): void {
    this.service.getGuidePackages().subscribe({
      next: (response) => {
  ...
  changeBookingStatus(status: number, id: string, val: number) {
    Swal.fire({
      title: `Are you sure you want to ${status === 2 ? 'Open' : 'Close'
        } this package`,
      showDenyButton: true,
